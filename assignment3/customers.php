    ...
    Class Customers{
        ...
        function create(){
            ...
            if ($this->validateInput()) {
                ...
                if($this->imageValidity){
                    $sql = "INSERT INTO customers (name,email,mobile,image,imagename,imagedescription,imagepath) values(?, ?, ?, ?, ?, ?, ?)";
                    $q = $pdo->prepare($sql);
                    $q->execute(array($this->getName(),$this->getEmail(),$this->getPhone(), $this->getImage(), $this->getImageName(), $this->getImageDescription(), $this->getImagePath()));
                } else{
        ...
        function validateImage($fileName, $tempFileName, $fileSize){
            ...
            if ($info === FALSE) {
               $valid = false;
               echo "Error Unable to determine <i>image</i> type of uploaded file";
            }
            ...
            if($fileSize > 500000) { 
                $valid = false;
                echo "Error: file exceeds 500kb";
            }
            
             $this->imageValidity = $valid;
             return $valid;
   
        }
        
        
        function setImage($tempFileName){
           
            // put the content of the file into a variable, $content 
            // and assign that to the fileContent variable in the 
            //customers object
